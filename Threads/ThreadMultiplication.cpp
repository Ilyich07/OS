...
public:
    ...
    int getExpectedThreads() const {
        int numBlocks = (matrixSize + blockSize - 1) / blockSize;
        return numBlocks * numBlocks * numBlocks;
    }
};
...
int main() {
    ...
    if (N <= 10) {
        ...
        std::cout << "\nMatrix size: " << N << "x" << N << " (too large to display)" << std::endl;
    }
    ...
    for (int blockSize = 1; blockSize <= N; blockSize++) {
        ...
        int expectedThreads = multiplier.getExpectedThreads();
    ...
    std::cout << "Simple algorithm time: " << simpleTimeMs << " ms" << std::endl;
    
    if (N <= 10) {
        std::cout << "\nMultiplication result:" << std::endl;
        resultSimple.print();
    } else {
        std::cout << "\nResult matrix " << N << "x" << N << " calculated successfully" << std::endl;
        std::cout << "First 3x3 elements:" << std::endl;
        for (int i = 0; i < std::min(3, N); ++i) {
            for (int j = 0; j < std::min(3, N); ++j) {
                std::cout << resultSimple(i, j) << " ";
            }
            std::cout << std::endl;
        }
    }
